...
def train(epoch, args, net, mi_net=None):
    ...
    return False, avg_epoch_loss, avg_mask_loss, snr_min, snr_max, snr_avg
...
def save_checkpoint(epoch, avg_loss, epoch_train_loss, mask_train_loss,
...
if __name__ == '__main__':
    ...
    for epoch in range(start_epoch, args.epochs):
        start = time.time()
        # Training
        interrupted, epoch_train_loss, mask_train_loss, snr_min, snr_max, snr_avg = train(
            epoch, args, deepsc)
        if interrupted:
            print(
                f"Training stopped at epoch {epoch + 1}. Saving checkpoint...")
            avg_loss = validate(epoch, args, deepsc, seq_to_text)
            save_checkpoint(epoch, avg_loss, epoch_train_loss, mask_train_loss,
                snr_min, snr_max, snr_avg)
            break
