...
# Training funtion
def train(epoch, args, net, mi_net=None, fgm=None):
    global stop_training
    ...
    for noise_sents, trg_sents, label_tensors in pbar:
        ...
        if mi_net is not None:
            ...
            # loss_total, snr = # Trong vòng lặp train
            loss_total, loss_adv, snr = train_step(
                net, noise_sents, trg_sents, noise_std,
                pad_idx, optimizer, criterion, args.channel,
                fgm=fgm  # truyền vào đây
            )
            epoch_loss += loss_total
            loss_adv_total += loss_adv
            snr_values.append(snr)
            pbar.set_description(
                f'Epoch: {epoch + 1}; Type: Train; Loss: {loss_total:.5f}; Loss_adv: {loss_adv:.5f}; SNR: {snr:.5f}; Noise Std: {noise_std:.5f}')
    ...
    avg_loss_adv = loss_adv_total / len(train_iterator)
    avg_mi_bits = mi_bits_total / batch_count if batch_count > 0 else 0
    return False, avg_epoch_loss, avg_loss_adv, avg_mi_bits, snr_min, snr_max, snr_avg
...
def save_checkpoint(epoch, avg_loss, epoch_train_loss, epoch_adv_loss,
...
if __name__ == '__main__':
    ...
    for epoch in range(start_epoch, args.epochs):
        start = time.time()
        # Training
        interrupted, epoch_train_loss, epoch_adv_loss, avg_mi_bits, snr_min, snr_max, snr_avg = train(
            epoch, args, deepsc, None, fgm)
        if interrupted:
            print(
