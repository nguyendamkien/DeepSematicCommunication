...
def train(epoch, args, net, fgm):
    ...
    for noise_sents, clean_sents, labels in pbar:
        if stop_training:
            return True, epoch_loss, epoch_bce_loss, epoch_adv_loss, min(
        ...
        snr_values.append(snr)
        pbar.set_description(
            f'Epoch: {epoch + 1}; Type: Train; Loss: {loss_total:.5f}; BCE: {bce_loss_val:.5f}; ADV: {loss_adv_val:.5f}; SNR: {snr:.5f}; Noise Std: {noise_std:.5f}')
    ...
    avg_epoch_adv_loss = epoch_adv_loss / len(train_iterator)
    # avg_mi_bits = mi_bits_total / batch_count if batch_count > 0 else 0
    return False, avg_epoch_loss, avg_epoch_bce_loss, avg_epoch_adv_loss, snr_min, snr_max, snr_avg
...
if __name__ == '__main__':
    ...
    fgm = FGM(ca_deepsc, epsilon=args.epsilon)
